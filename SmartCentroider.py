...
class SmartCentroider(object):
    ...
    def __init__(self, filelist, bands=None):
        ...
        self.npix_per_cluster_cut = (1,1e9)
...
#        tof_fileset = self.filelist
        ...
        if self.n_tof_files >= n_files: #if there fewer files that the nominal, take them all
            ...
            if self.DEBUG >1: print ('selected %s files for ToF'%len(tof_fileset));
            sys.stdout.flush()

        tof_imgs = []
        for filename in tof_fileset: # load images
            tof_imgs.append(fn.TimepixFileToImage(filename, skiplines=self.skiplines))
    ...
    def CalculateBands(self):
        ...
        if self.DEBUG>=3: print ('Found %s peaks at %s'%(len(self.peaks), self.peaks));
        sys.stdout.flush()
    ...
    def ShowBands(self):
        ...
        if self.DEBUG>=1:
            ...
            print ('Bands set at %s'%self.bands);
            sys.stdout.flush()
        pl.show()
    ...
    def FindClusters(self):
        if self.DEBUG>=2: print ('Using %s bands...'%len(self.bands));
        sys.stdout.flush()
        ...
        for filenum, filename in enumerate(self.filelist):
            if filenum%500==0: print ('Smart centroided %s of %s files...'%(filenum, len(self.filelist)));
            sys.stdout.flush()
            ...
            img = fn.TimepixFileToImage(filename, skiplines=self.skiplines)

            segmentation, segments = ndimage.label(img, struct_el) # find clusters
            if self.DEBUG>2: print ('Found %s clusters without using band information'%segments);
            sys.stdout.flush()
            ...
            for bandnum,(tmin, tmax) in enumerate(self.bands): # Process each band in turn
                ...
                seg_sum += segments
                if self.DEBUG>=3: print ('Found %s segs in band %s (%s - %s)'%(segments, bandnum, tmin, tmax));
                sys.stdout.flush()
                ...
                if self.DEBUG>=2: print ('CoMs for band %s: %s'%(bandnum, CoMs));
                sys.stdout.flush()
            #     index = (np.asarray([_[0] for _ in CoMs]),np.asarray([[_[1] for _ in CoMs]]))
            #     codes = img[CoMs]

            if self.DEBUG>=2: print ('Found %s clusters when using bands'%seg_sum);
            sys.stdout.flush()
        print ('Finished smart centroiding %s files...'%(len(self.filelist)));
        sys.stdout.flush()
...
#         now = time.time()
        for band_num, t_range in enumerate(custom_bands): # loop over bands
            if self.DEBUG>=3:print ('Processing band %s of %s'%(band_num+1, len(bands)));
            sys.stdout.flush()
